...
static int dbmd4_spi_reset_post_pll_divider(struct dbmdx_private *p)
{
	...
	if (!(spi_p->post_pll_div & DBMDX_POST_PLL_DIV_MASK)) {
		dev_dbg(spi_p->dev,
			"%s: No need to reset POST PLL divider, already 0\n",
			__func__);
		return 0;
	}
...
static int dbmd4_spi_restore_post_pll_divider(struct dbmdx_private *p)
{
	...
	if (!(spi_p->post_pll_div & DBMDX_POST_PLL_DIV_MASK)) {
		dev_dbg(spi_p->dev,
			"%s: No need to restore POST PLL divider, already 0\n",
			__func__);
		return 0;
	}
...
static int dbmd4_reconfigure_dsp_clock(struct dbmdx_private *p, int index)
{
	...
	ret = send_spi_cmd_va(p,
		DBMDX_REGN_DSP_CLOCK_CONFIG | (dsp_clock_cfg & 0xffff),
		NULL);
	...
	ret = send_spi_cmd_va(p,
			DBMDX_REGN_FIRST_MICROPHONE_CONFIG | mic1_val,
			NULL);
	...
	ret = send_spi_cmd_va(p,
			DBMDX_REGN_SECOND_MICROPHONE_CONFIG | mic2_val,
			NULL);
