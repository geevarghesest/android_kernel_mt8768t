...
#include <../../../drivers/misc/mediatek/sensors-1.0/sar/sx932x.h>
extern psx93XX_t this ;
extern int sxsar_calibration(psx93XX_t this);
//extern struct aw9610x *aw9610x;
extern int sar_flag;
extern struct aw9610x *aw9610x;
extern int saraw_flag;
extern int awsar_calibration(struct aw9610x *aw9610x);
...
static void send_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_NO_MIC:
		...
		pr_info("THE sarflag IS %d\n",sar_flag);
		if(sar_flag==1)
			{
					pr_info("THE SAR IS sx9328 \n");
					sxsar_calibration(this);
			}
		pr_info("THE sarawflag IS %d\n",saraw_flag);
		if(saraw_flag==2)
			{
					pr_info("THE SAR IS aw9610 \n");
					awsar_calibration(aw9610x);
			}
