...
static struct snd_soc_dai_link mt_soc_dai_common_l[] = {
	/* FrontEnd DAI Links */
	{
		.name = "MultiMedia1",
...
static struct snd_soc_dai_link_component aw8838_dai_link_component[] =
{
	{
		.name= "aw8838_l_smartpa",
		.dai_name="aw8838_l-aif",
	},
	{
		.name= "aw8838_r_smartpa",
		.dai_name="aw8838_r-aif",
	},
};
...
static struct snd_soc_dai_link mt_soc_extspk_dai[] = {
	{
		...
		.dai_fmt = SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_CBS_CFS |
			   SND_SOC_DAIFMT_NB_NF,
		.ops = &cs35l35_ops,
#elif defined(CONFIG_SND_SMARTPA_AW8838)
		//.codec_dai_name = "aw8838_l-aif",
		//.codec_name = "aw8838_l_smartpa",
		.codecs = aw8838_dai_link_component,
		.num_codecs = 2,
#else
		.codec_dai_name = "snd-soc-dummy-dai",
		.codec_name = "snd-soc-dummy",
...
static struct snd_soc_dai_link mt_soc_extspk_dai_l[] = {
	{
		.name = "ext_Speaker_Multimedia",
		.stream_name = MT_SOC_SPEAKER_STREAM_NAME,
		.cpu_dai_name = "snd-soc-dummy-dai",
...
static struct snd_soc_dai_link
	mt_soc_dai_component_l[ARRAY_SIZE(mt_soc_dai_common_l) +
#ifdef CONFIG_SND_SOC_MTK_BTCVSD
			     ARRAY_SIZE(mt_soc_btcvsd_dai) +
#endif
			     ARRAY_SIZE(mt_soc_exthp_dai) +
			     ARRAY_SIZE(mt_soc_extspk_dai_l)];
...
static struct snd_soc_card mt_snd_soc_card_mt_l = {
	.name = "mt-snd-card",
	.dai_link = mt_soc_dai_common_l,
	.num_links = ARRAY_SIZE(mt_soc_dai_common_l),
};
...
static int mt_soc_snd_probe(struct platform_device *pdev)
{
    ...
    struct snd_soc_card *card_l = &mt_snd_soc_card_mt_l;
#ifdef CONFIG_SND_SOC_MTK_BTCVSD
    ...
    if (ProjectName == 0) {
	    ...
	    pr_debug("%s(), dai_link = %p\n",
		      __func__, mt_snd_soc_card_mt_l.dai_link);
	/* DEAL WITH DAI LINK */
		memcpy(mt_soc_dai_component_l, mt_soc_dai_common_l,
			   sizeof(mt_soc_dai_common_l));
		daiLinkNum += ARRAY_SIZE(mt_soc_dai_common_l);
		...
		memcpy(mt_soc_dai_component_l + daiLinkNum,
		mt_soc_btcvsd_dai, sizeof(mt_soc_btcvsd_dai));
		daiLinkNum += ARRAY_SIZE(mt_soc_btcvsd_dai);
	#endif

		memcpy(mt_soc_dai_component_l + daiLinkNum, mt_soc_exthp_dai,
			   sizeof(mt_soc_exthp_dai));
		daiLinkNum += ARRAY_SIZE(mt_soc_exthp_dai);

		memcpy(mt_soc_dai_component_l + daiLinkNum, mt_soc_extspk_dai_l,
			   sizeof(mt_soc_extspk_dai_l));
		daiLinkNum += ARRAY_SIZE(mt_soc_extspk_dai_l);

        mt_snd_soc_card_mt_l.dai_link = mt_soc_dai_component_l;
		mt_snd_soc_card_mt_l.num_links = daiLinkNum;

		card_l->dev = &pdev->dev;
		platform_set_drvdata(pdev, card_l);

		ret = devm_snd_soc_register_card(&pdev->dev, card_l);
		if (ret)
