...
/*Add for OTA disable power key long press start*/
/*add kpdpwr_reset node*/
unsigned long kpdpwr_status;
const struct mtk_pmic_regs *pmic_regs_2;
struct mtk_pmic_keys *keys_2;
static struct platform_driver pmic_keys_pdrv;
/*Add for OTA disable power key long press  end*/
...
extern void bq25601_set_batfet_reset_enable(unsigned int val);
/*Add for OTA disable power key long press  start*/
		...
		const char *buf, size_t count)
{
	...
	if(kpdpwr_status){
		...
		bq25601_set_batfet_reset_enable(0);
		regmap_update_bits(keys_2->regmap, pmic_rst_reg,
				   pwrkey_rst,
				   0);
		regmap_update_bits(keys_2->regmap, pmic_rst_reg,
