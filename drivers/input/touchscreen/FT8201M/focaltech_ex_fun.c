...
bool FTS_status;//add for tp_work need
...
/*add for tp_info start*/
static ssize_t fts_tp_info_read( struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
	int ret=0;
	int cnt=0;
	char *page = NULL;
	...
	r1 = strcmp(mtkfb_lcm_name,"ft8201_wxga_vdo_incell_boe");
	r2 = strcmp(mtkfb_lcm_name,"ft8201AB_wxga_vdo_inx");
	r3 = strcmp(mtkfb_lcm_name,"ft8201M_wxga_vdo_boe");
	if(!r1){
		ret=fts_read_reg(FTS_REG_FW_VER, &fwver);
		if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00))
			snprintf(tp_ver_show_str, 80, "[Vendor]Focaltech,[FW]unknown,[IC]ft8201\n");
		else
			snprintf(tp_ver_show_str,80,"[Vendor]Focaltech,[FW]0x%02x,[IC]ft8201\n",fwver);
	}else if(!r2){
		ret=fts_read_reg(FTS_REG_FW_VER, &fwver);
		if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00))
			snprintf(tp_ver_show_str, 80, "[Vendor]Focaltech,[FW]unknown,[IC]ft8201AB\n");
		else
			snprintf(tp_ver_show_str,80,"[Vendor]Focaltech,[FW]0x%02x,[IC]ft8201AB\n",fwver);
	}else if(!r3){
		ret=fts_read_reg(FTS_REG_FW_VER, &fwver);
		if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00))
			snprintf(tp_ver_show_str, 80, "[Vendor]Focaltech,[FW]unknown,[IC]ft8201M\n");
		else
			snprintf(tp_ver_show_str,80,"[Vendor]Focaltech,[FW]0x%02x,[IC]ft8201M\n",fwver);
	}else{
		ret=fts_read_reg(FTS_REG_FW_VER, &fwver);
		if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00))
			snprintf(tp_ver_show_str, 80, "[Vendor]Focaltech,[FW]unknown,[IC]ft8201M--\n");
...
/*add tp_work start */
static ssize_t fts_tp_work_write(struct file *filp,const char __user *buf, size_t size, loff_t *ppos)
{
	char tmp_data[64] = {0};
	if(copy_from_user(tmp_data, buf, size)) {
		FTS_ERROR("copy_from_user() fail.\n");
		return -EFAULT;
	}
	if(strncmp(tmp_data, "1",1) == 0)
		{
			fts_release_all_finger();
			fts_irq_enable();
			FTS_status= true;
		}
	if(strncmp(tmp_data, "0",1) == 0)
		{
			fts_irq_disable();
			fts_release_all_finger();
			FTS_status=false;
...
static ssize_t fts_tp_work_read(struct file *file,  char __user *buf, size_t size, loff_t *ppos)
{
	ssize_t cnt=0;
	char *page = NULL;
	...
	if (IS_ERR_OR_NULL(page))
		return -ENOMEM;
	cnt = sprintf(page, "%s", (FTS_status ? "1\n" : "0\n"));
	cnt = simple_read_from_buffer(buf, size, ppos, page, cnt);
	if (*ppos != cnt)
		*ppos = cnt;
	FTS_INFO("Touchpad status : %s", page);
...
int lct_fts_tp_selftest_callback(unsigned char cmd)
{
    ...
    case TP_SELFTEST_CMD_LONGCHEER_MMI:
        ...
        break;
    case TP_SELFTEST_CMD_I2C:
        ret = lct_tp_selftest_all();//PASS
        break;
    case TP_SELFTEST_CMD_OPEN:
        ret = lct_tp_selftest_all();
        break;
    case TP_SELFTEST_CMD_SHORT:
        ret = lct_tp_selftest_all();
        break;
    default:
        break;
...
/*add for tp_selftest end*/

/*add for tp_info start*/
static const struct file_operations tp_info_proc_fops = {
	.read = fts_tp_info_read,
};

static struct proc_dir_entry *proc_entry_tp_info;
/*
static struct proc_dir_entry *proc_entry_tp_selftest;//add for tp_selftest proc

static const struct file_operations tp_selftest_proc_fops = {
	.read= fts_tp_selftest_read,
	.write= fts_tp_selftest_write,
};//add for tp_selftest proc
*/
static struct proc_dir_entry *proc_entry_tp_work;

static const struct file_operations tp_work_proc_fops = {
    .read= fts_tp_work_read,
	.write= fts_tp_work_write,
};

static int tp_fm_creat_proc_entry(void)
{
	proc_entry_tp_info = proc_create("tp_info", 0444, NULL, &tp_info_proc_fops);
	lct_tp_selftest_init(lct_fts_tp_selftest_callback);//add for tp_selftest
	proc_entry_tp_work = proc_create("tp_work", 0666, NULL, &tp_work_proc_fops);//add for tp_work

	if (IS_ERR_OR_NULL(proc_entry_tp_info))
		FTS_ERROR("add /proc/tp_info error \n");
	return 0;
}

static int tp_fm_remove_proc_entry(void)
{
	proc_remove(proc_entry_tp_info);
	//proc_remove(proc_entry_tp_selftest);//add for tp_selftest
	proc_remove(proc_entry_tp_work);//add for tp_work
	return 0;
}
/*add for tp_info end*/
...
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
	...
	tp_fm_creat_proc_entry();//add for tp_info
    if (ret) {
...
{
    sysfs_remove_group(&ts_data->dev->kobj, &fts_attribute_group);
	tp_fm_remove_proc_entry();//add for tp_info
    return 0;
}
