...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "focaltech_ts_fw_boe"
#define FTS_FW_NAME_PREX_WITH_REQUEST2               "focaltech_ts_fw_inx"
#define FTS_FW_NAME_PREX_WITH_REQUEST3               "focaltech_ts_fw_boe_m"
#define FTS_FW_NAME_PREX_WITH_REQUEST4               "focaltech_ts_fw_boe_m--"
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    ...
    if(!r1){
			...
			snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin", \
				FTS_FW_NAME_PREX_WITH_REQUEST2, \
				upg->module_info->vendor_name);
			}
	else if(!r3){
			snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin", \
				FTS_FW_NAME_PREX_WITH_REQUEST3, \
				upg->module_info->vendor_name);
		}
	else{
			snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin", \
				FTS_FW_NAME_PREX_WITH_REQUEST4, \
				upg->module_info->vendor_name);
	}
