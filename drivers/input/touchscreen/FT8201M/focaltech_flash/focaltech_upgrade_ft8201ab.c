...
/*****************************************************************************
*
* File Name: focaltech_upgrade_ft8201ab.c
*
...
*****************************************************************************/
u8 pb_file_ft8201ab[] = {
#include "../include/pramboot/FT8201AB_Pramboot_V1.1_20200821.i"
};
...
static int fts_ft8201ab_get_hlic_ver(u8 *initcode)
{
...
static int fts_ft8201ab_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
    int ret = 0;
    u32 start_addr = 0;
    u8 cmd[4] = { 0 };
    ...
    if (upgrade_func_ft8201ab.appoff_handle_in_ic) {
        start_addr = 0; /* offset handle in pramboot */
    } else {
        start_addr = upgrade_func_ft8201ab.appoff;
    }
...
/************************************************************************
* Name: fts_ft8201ab_upgrade
* Brief:
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft8201ab_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 app_len = 0;
    ...
    app_len = len - upgrade_func_ft8201ab.appoff;
    tmpbuf = buf + upgrade_func_ft8201ab.appoff;
    ret = fts_ft8201ab_upgrade_mode(FLASH_MODE_APP, tmpbuf, app_len);
    if (ret < 0) {
...
static int fts_ft8201ab_lic_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 lic_len = 0;
    ...
    ret = fts_ft8201ab_upgrade_mode(FLASH_MODE_LIC, tmpbuf, lic_len);
    if (ret < 0) {
...
struct upgrade_func upgrade_func_ft8201ab = {
    .ctype = {0x21},
    .fwveroff = 0x110E,
    .fwcfgoff = 0x0F80,
    .appoff = 0x1000,
    ...
    .pramboot = pb_file_ft8201ab,
    .pb_length = sizeof(pb_file_ft8201ab),
    .upgrade = fts_ft8201ab_upgrade,
    .get_hlic_ver = fts_ft8201ab_get_hlic_ver,
    .lic_upgrade = fts_ft8201ab_lic_upgrade,
};
