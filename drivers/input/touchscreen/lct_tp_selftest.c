...
#define TP_SELF_TEST_I2C_CMD      "i2c"
#define TP_SELF_TEST_OPEN_CMD     "open"
#define TP_SELF_TEST_SHORT_CMD    "short"

enum lct_tp_selftest_cmd {
	TP_SELFTEST_CMD_LONGCHEER_MMI = 0x00,
	TP_SELFTEST_CMD_I2C = 0x01,
	TP_SELFTEST_CMD_OPEN = 0x02,
	TP_SELFTEST_CMD_SHORT = 0x03,
};
...
static ssize_t tp_selftest_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	} else if ((strncmp(tmp_data, TP_SELF_TEST_I2C_CMD, strlen(TP_SELF_TEST_I2C_CMD)) == 0)
			&& (!is_in_self_test)) { //i2c

		TP_LOGW("TP i2c self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_I2C;


	} else if ((strncmp(tmp_data, TP_SELF_TEST_OPEN_CMD, strlen(TP_SELF_TEST_OPEN_CMD)) == 0)
			&& (!is_in_self_test)) { //open

		TP_LOGW("TP open self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_OPEN;

	} else if ((strncmp(tmp_data, TP_SELF_TEST_SHORT_CMD, strlen(TP_SELF_TEST_SHORT_CMD)) == 0)
			&& (!is_in_self_test)) { //short

		TP_LOGW("TP short self-test ...\n");
		lct_tp_selftest_cmd = TP_SELFTEST_CMD_SHORT;
