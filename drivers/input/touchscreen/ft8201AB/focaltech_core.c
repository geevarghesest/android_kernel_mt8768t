...
static int fts_match_cid(struct fts_ts_data *ts_data,
                         u16 type, u8 id_h, u8 id_l, bool fw_valid)
{
    ...
    for (i = 0; i < cid_entries; i++) {
        if (fw_valid && (type == chip_id_list[i].type)) {
            break;
        } else if (!fw_valid && (type == chip_id_list[i].type)) {
    ...
    for (i = 0; i < ctype_entries; i++) {
        if (VALID == fw_valid) {
            if (((id_h == ctype[i].chip_idh) && (id_l == ctype[i].chip_idl))
                || (!fts_match_cid(ts_data, ctype[i].type, id_h, id_l, VALID)))
                break;
        } else {
            if (((id_h == ctype[i].rom_idh) && (id_l == ctype[i].rom_idl))
                || ((id_h == ctype[i].pb_idh) && (id_l == ctype[i].pb_idl))
                || ((id_h == ctype[i].bl_idh) && (id_l == ctype[i].bl_idl))) {
                fts_match_cid(ts_data, ctype[i].type, id_h, id_l, INVALID);
                break;
            }
        }
    }
