...
static u8 ft8201_weakshort_get_adcdata(struct ft8201_info *info, int read_num, int *rbuf)
{
    int ret = 0;
...
static void judge_node_is_mux(int *data, int *min2, int *max2, int *min1, int *max1, int ini_min)
{
    int i = 0;
    int j = 0;
...
static int get_order_incell(u16 saddr, int byte_num, int *cb_buf)
{
    ...
    FTS_TEST_INFO("order packet:%d,remainder:%d", packet_num, packet_remainder);
...
static int ft8201_get_tx_rx_order(struct ft8201_info *info, int read_num, int *read_buffer)
{
    int ret = 0;
    int *buffer_master = NULL;
    int *buffer_slave = NULL;
    ...
    FTS_TEST_FUNC_ENTER();
    if (chip_have_two_heart(info)) {
        buffer_master = fts_malloc((master_tx + 1) * master_rx * 2 * sizeof(int));
        if (NULL == buffer_master) {
            FTS_TEST_SAVE_ERR("%s:master buf malloc fail\n", __func__);
            ret = -ENOMEM;
            goto GET_ORDER_ERR;
        ...
        buffer_slave = fts_malloc((slave_tx + 1) * slave_rx * 2 * sizeof(int));
        if (NULL == buffer_slave) {
            FTS_TEST_SAVE_ERR("%s:slave buf malloc fail\n", __func__);
            ret = -ENOMEM;
            goto GET_ORDER_ERR;
        ...
        /* master cb */
        work_as_master(info);
        ret = get_order_incell(0, (master_tx * master_rx  + 6) * 2, buffer_master);
        if (ret ) {
        ...
        /* slave cb */
        work_as_slave(info);
        ret = get_order_incell(0, (slave_tx * slave_rx + 6) * 2, buffer_slave);
        if (ret ) {
...
static int ft8201_get_mux_open_data(struct ft8201_info *info, struct fts_test *tdata,
                                    u8 reg20_val, u8 reg2e_val, u8 data_type, int *data)
{
    u8 tmp_val = 0;
    int ret = 0;
    int byte_num = 0;
...
static int ft8201_mux_open_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    u8 reg20_val = 0;
    ...
    judge_node_is_mux(orderdata, mux2_open_min, mux2_open_max, mux1_open_min, mux1_open_max, min);
    ...
    ret = ft8201_get_mux_open_data(info, tdata, reg20_val, reg2e_val, 2, mux_open_data_tmp);
    if (ret < 0) {
    ...
    /* compare */
    tmp_result = compare_array(mux_open_data_tmp, mux2_open_min, mux2_open_max, false);

    mux_open_data_cnt += tdata->node.tx_num * tdata->node.rx_num;

    /* disable mux1&3 */
    mux_open_data_tmp = mux_open_data + mux_open_data_cnt;
    ret = ft8201_get_mux_open_data(info, tdata, reg20_val, reg2e_val, 4, mux_open_data_tmp);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("get mux open data fail\n");
        goto restore_reg;
    }
    ...
    tmp_result = compare_array(mux_open_data_tmp, mux1_open_min, mux1_open_max, false);
...
test_err:
    ...
    /* save data */
    fts_test_save_data("Mux Open Test", CODE_MUX_OPEN_TEST,
                       mux_open_data, mux_open_data_cnt, false, false, *test_result);
...
static int ft8201_lcdnoise_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    ret = ft8201_write_reg(info, FACTORY_NOISE_MODE_REG, 0x64);
    if (ret < 0) {
