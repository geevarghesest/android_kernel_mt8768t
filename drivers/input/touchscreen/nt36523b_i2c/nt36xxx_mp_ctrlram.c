 ...
 *
 * $Revision: 60130 $
 * $Date: 2020-04-10 17:43:33 +0800 (週五, 10 四月 2020) $
 *
...
#define PEN_FW_RAW_TEST_CSV_FILE "/data/vendor/tmp/PenFWRawTest.csv"
#define PEN_NOISE_TEST_CSV_FILE "/data/vendor/PenNoiseTest.csv"
...
return:
	n.a.
*******************************************************/
static void print_selftest_result_on_log(int32_t TestResult,
...
int lct_nvt_tp_selftest_callback(unsigned char cmd)
{
	...
	case TP_SELFTEST_CMD_SHORT:
		...
		break;
	default:
		NVT_LOG("touch selftest command: unknown\n");
		NVT_LOG("--\n");
		return 0;//Unknown
	}
	...
	//Short Test Result
	print_selftest_result_on_log(TestResult_Short,
					RecordResult_Short, RawData_Short, X_Channel, Y_Channel);

	//Open Test Result
	print_selftest_result_on_log(TestResult_Open,
					RecordResult_Open, RawData_Open, X_Channel, Y_Channel);

	//FW Rawdata Test Result
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		 print_selftest_result_on_log(TestResult_FWMutual,
				RecordResult_FWMutual, RawData_FWMutual, X_Channel, Y_Channel);
	} else { // TestResult_FW_Rawdata is -1
		...
		if (TestResult_FW_CC == -1) { //FW CC
			print_selftest_result_on_log(TestResult_FW_CC,
	...
	//Noise Test Result
	if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
		print_selftest_result_on_log(TestResult_FW_DiffMax,
		...
		if (TestResult_FW_DiffMin == -1) { //FW Diff Min
			print_selftest_result_on_log(TestResult_FW_DiffMin,
