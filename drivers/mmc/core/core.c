...
{
	unsigned int sz;
	unsigned int erase_value = 262144; //256k
	...
	if (mmc_card_sd(card) && card->ssr.au) {
		if (card->ssr.au < erase_value) {//Modify for 256G SDCARD of TOSHIBA
			card->pref_erase = erase_value;
			card->erase_shift = ffs(erase_value) - 1;
		} else {
