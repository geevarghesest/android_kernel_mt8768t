...
#define DEPTH_OV02B_IIII_VENDOR_ID 0x07
#define DEPTH_OV02B_III_VENDOR_ID  0x41
#define DEPTH_OV02B_II_VENDOR_ID   0x42

#define DEPTH_OV02B_IIII_LENS_ID   0x94
#define DEPTH_OV02B_V_LENS_ID      0x04
...
static struct imgsensor_info_struct imgsensor_info = {
    ...
    .custom1 = {
            .pclk = 16500000,            //record different mode's pclk
            .linelength = 448,            //record different mode's linelength
            .framelength = 1528,            //record different mode's framelength
            .startx = 0,                    //record different mode's startx of grabwindow
            .starty = 0,                    //record different mode's starty of grabwindow
            .grabwindow_width = 1600,        //record different mode's width of grabwindow
            .grabwindow_height = 1200,        //record different mode's height of grabwindow
...
/*
static kal_uint16 read_vendor_id_ov02b(void )
{
   kal_uint16 vendor_id0 = 0;
   kal_uint16 vendor_id1 = 0;
   kal_uint16 vendor_id2 = 0;

   kal_uint16 lens_id0 = 0;
   kal_uint16 lens_id1 = 0;
   //change to pate 6
   ...
   vendor_id0 =  read_cmos_sensor(0x11);
   vendor_id1 =  read_cmos_sensor(0x18);
   vendor_id2 =  read_cmos_sensor(0x19);
   pr_err("%s %d depth_matched buqingshuai = %x  || %x || %x\n",__func__, __LINE__,vendor_id0,vendor_id1,vendor_id2);
   if((vendor_id1 == DEPTH_OV02B_IIII_VENDOR_ID) || ( vendor_id0 == DEPTH_OV02B_IIII_VENDOR_ID )){
        lens_id0 = read_cmos_sensor(0x12);
        lens_id1 = read_cmos_sensor(0x19);
        pr_err("G-Dg ofilm depth_matched 0x%02X, 0x%02X\n", lens_id0, lens_id1);
        if ((lens_id1 == DEPTH_OV02B_IIII_LENS_ID) || (lens_id0 == DEPTH_OV02B_IIII_LENS_ID)) {
            return 4;
        } else if ((lens_id1 == DEPTH_OV02B_V_LENS_ID) || (lens_id0 == DEPTH_OV02B_V_LENS_ID)) { //ofilm csp
            return 5;
        } else {
            return -1;
        }
   } else if((vendor_id1 == DEPTH_OV02B_III_VENDOR_ID) || ( vendor_id0 == DEPTH_OV02B_III_VENDOR_ID ))
   {
		return 3; 
   } else if((vendor_id2 == DEPTH_OV02B_II_VENDOR_ID) || (vendor_id0 == DEPTH_OV02B_II_VENDOR_ID))
   {
       return 2;
   } else
   {
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if (5 != flag){
        pr_err("%s %d ov02b_v match vendor id failed\n",__func__, __LINE__);
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        LOG_INF("Read id fail,info sensor id : 0x%x, sensor id: 0x%x\n",
