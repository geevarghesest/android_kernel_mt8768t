...
kal_uint8 ov8856_2a_data[8];
kal_uint8 ov8856_lsc_data[240];
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	/* Preview */
	{ 3296, 2480, 0, 12, 3296, 2456, 1648, 1228, 8,
	2, 1632, 1224, 0, 0, 1632, 1224},
	/* capture */
	{ 3296, 2480, 0, 12, 3296, 2456, 3296, 2456, 16,
	4, 3264, 2448, 0, 0, 3264, 2448},
	/* video*/
	{ 3296, 2480, 0, 12, 3296, 2456, 3296, 2456, 16,
	4, 3264, 2448, 0, 0, 3264, 2448},
	/*hight speed video */
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	case IMAGE_NORMAL:
			...
			write_cmos_sensor(0x3821, ((read_cmos_sensor(0x3821) & 0xB9) | 0x46));
	...
	case IMAGE_H_MIRROR:
			write_cmos_sensor(0x3820, ((read_cmos_sensor(0x3820) & 0xB9) | 0x00));
			write_cmos_sensor(0x3821, ((read_cmos_sensor(0x3821) & 0xB9) | 0x00));
			write_cmos_sensor(0x502e, ((read_cmos_sensor(0x502e) & 0xFC) | 0x03));
			write_cmos_sensor(0x5001, ((read_cmos_sensor(0x5001) & 0xFB) | 0x00));
...
kal_uint16 addr_data_pair_init_ov8856[] = {
	...
	0x3830, 0x06,
	0x3836, 0x02,
	0x3837, 0x10,	//AM12A updated
	0x3862, 0x04,
	0x3863, 0x08,
	0x3cc0, 0x33,
	0x3d85, 0x14,	//AM12A updated
	0x3d8c, 0x73,
	0x3d8d, 0xde,
	0x4001, 0xe0,
	0x4003, 0x40,
...
{
	u8 flag;
	u16 temp, i, addr, checksum_offset;
	u32 checksum_cal;
	...
	write_cmos_sensor(0x3d88, 0x70); // OTP start address 0x7010
	write_cmos_sensor(0x3d89, 0x10);
	write_cmos_sensor(0x3d8A, 0x72); // OTP end address 0x720f
	...
	if ((flag & 0xc0) >> 6 == 0x01) {
		addr = 0x7013;
	} else if ((flag & 0x30) >> 4 == 0x01) {
		addr = 0x701d;
	}

	if (addr != 0) {
		for (i = 0; i < 8; i++) { // 5 wb + 3 af
			ov8856_2a_data[i] = read_cmos_sensor(addr+i);
			//LOG_INF("G-Dg 2a data=0x%02X, %d\n", ov8856_2a_data[i], i);
		}
		otp_status = 1;
	} else {
		otp_status = 0;
	...
	addr = 0;
	checksum_cal = 0;
	flag = read_cmos_sensor(0x7028);
	if ((flag & 0xc0) >> 6 == 0x01) {
		addr = 0x7029;
		checksum_offset = 0x7119;
	} else if ((flag & 0x30) >> 4 == 0x01) {
		addr = 0x711a;
		checksum_offset = 0x720a;
	}

	if (addr != 0) {
		for (i = 0; i < 240; i++) {
			ov8856_lsc_data[i] = read_cmos_sensor(addr+i);
			checksum_cal += ov8856_lsc_data[i];
			//LOG_INF("G-Dg lsc data=0x%02X, %d, checksum=0x%04X\n", ov8856_lsc_data[i], i, checksum_cal);
		}
		checksum_cal = (checksum_cal)%255 + 1;

		if (checksum_cal != read_cmos_sensor(checksum_offset)) {
			pr_err("G-Dg lsc checksum fail, 0x%04X 0x%04X\n", checksum_cal, read_cmos_sensor(checksum_offset));
	...
	//set 0x5001[3] to '1'
	temp = read_cmos_sensor(0x5001);
	write_cmos_sensor(0x5001, (0x08 & 0x08) | (temp & (~0x08)));
}

static void format_lsc_data(void)
{
	u16 i, temp;

	// Load Lenc default setting
	write_cmos_sensor(0x59f0, 0x80);
	write_cmos_sensor(0x59f1, 0x40);
	write_cmos_sensor(0x59f2, 0x40);
	write_cmos_sensor(0x59f3, 0x18);
	write_cmos_sensor(0x59f4, 0x32);
	write_cmos_sensor(0x59f5, 0x02);
	write_cmos_sensor(0x59f6, 0x7c);
	write_cmos_sensor(0x59f7, 0x01);
	...
	for (i = 0; i < 240; i++) {
		write_cmos_sensor(0x5900+i, ov8856_lsc_data[i]);
	}
}
