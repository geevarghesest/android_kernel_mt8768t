...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[4] = {
    ...
    {2592, 1944, 0, 242, 2592, 1460, 2592, 1460, 0000, 0000, 2592, 1460, 0, 0, 2592, 1460},    /* video */
    {2592, 1944, 0, 242, 2592, 1460, 2592, 1460, 0000, 0000, 2592, 1460, 0, 0, 2592, 1460}    /* hight speed video */
};
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor_8(0x3400,    0x00);    //lsc on
...
static void read_otp_all_data(void)
{
    ...
    write_cmos_sensor(0x0136, 0x18);    // write pll 24mHZ
    write_cmos_sensor(0x0100, 0x01);    // stream on
    mdelay(50);                         // delay 50 ms

    // check lsc,wb flag.
    // page 20, group1 0x0a04, group2 0x0a05
    addr = 0;
    write_cmos_sensor(0x0A02, 0x14); // Select the page to write by writing to 0xD0000A02 0x01~0x14
    ...
    if ((flag & 0xc0) >> 6 == 0x01) {
        ...
        if ((flag & 0xc0) >> 6 == 0x01) {
            ...
            pr_err("G-Dg lsc,wb flag error\n");
        }
    }

    LOG_INF("G-Dg wb addr=0x%04X\n", addr);
    if (addr != 0) {
        for (i = 0; i < 18; i++) { // read wb data
            s5k5e9_2a_data[i] = read_cmos_sensor(addr+i);
            LOG_INF("G-Dg wb data=0x%02X, %d\n", s5k5e9_2a_data[i], i);
        }
        otp_status = 1;
    } else {
        otp_status = 0;
    ...
    // check info,af flag.
    // page 17 group1 0x0a04, page18 group2 0x0a04, page19 group3 0x0a04
    addr = 0;
    for (i = 0x11; i <= 0x13; i++) {
        write_cmos_sensor(0x0A02, i); // select page
        write_cmos_sensor(0x0A00, 0x01); // read mode
        flag = read_cmos_sensor(0x0A04);
        if ((flag & 0xc0) >> 6 == 0x01) {
            addr = 0x0a15;
            break;
    ...
    if (addr != 0) {
        for (i = 0; i < 7; i++) { // read af data
            s5k5e9_2a_data[i+18] = read_cmos_sensor(addr+i);
            LOG_INF("G-Dg af data=0x%02X, %d\n", s5k5e9_2a_data[i+18], i);
        }
        otp_status = 1;
    } else {
        otp_status = 0;
    ...
    write_cmos_sensor(0x0A00, 0x00); //Reset the NVM interface by writing 00h to 0xD0000A00
}
