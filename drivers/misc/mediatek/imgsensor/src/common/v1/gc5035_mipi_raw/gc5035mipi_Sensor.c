...
{
	kal_uint8 flag;
	kal_uint16 i, addr, checksum_cal;

	addr = 0;
	checksum_cal = 0;

	for (i = 0x1a00; i <= 0x1c30; i += 35*8) {
		flag = gc5035_otp_read_byte(i);
		if ((flag & 0xc0) >> 6 == 0x01) {
			addr = i;
			break;
		}
	...
	//pr_err("G-Dg addr=0x%04X\n", addr);
	if (addr != 0) {
		gc5035_otp_read_group(addr + 1*8, &gc5035_2a_data[0], sizeof(gc5035_2a_data));

		for(i = 0; i < sizeof(gc5035_2a_data); i++) {
			checksum_cal += gc5035_2a_data[i];
			//pr_err("G-Dg data=0x%02X, %d, checksum_cal=0x%04X\n", gc5035_2a_data[i], i, checksum_cal);
		}
		checksum_cal = checksum_cal % 255 + 1;

		if (checksum_cal != gc5035_otp_read_byte(addr+34*8)) {
			pr_err("G-Dg gc5035 checksum fail, checksum_cal=0x%02X, read_checksum=0x%02X\n",
						checksum_cal, gc5035_otp_read_byte(addr+34*8));
			for (i = 0; i < sizeof(gc5035_2a_data); i++) {
				gc5035_2a_data[i] = 0;
			}
			otp_status = 0;
