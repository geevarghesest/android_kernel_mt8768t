...
#define LOG_1 LOG_INF("s5k4h7yx,MIPI 2LANE\n")
#define LOG_2 \
...
extern unsigned int otp_status;

#define S5K4H7YX_OTP 0
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_8(0x392F, 0x01);  //1130-DF MIPI
	write_cmos_sensor_8(0x3930, 0x80);  //1130-DF MIPI
...
static void read_otp_all_data(void)
{
	kal_uint8 flag, i;
	kal_uint16 addr, checksum_cal;

	LOG_INF("G-Dg Enter\n");

	write_cmos_sensor(0x0136, 0x18);	// 24MHz
	write_cmos_sensor(0x0137, 0x00);
	write_cmos_sensor(0x0305, 0x06);	// PLL pre div
	write_cmos_sensor(0x0306, 0x00);	//PLL multiplier
	write_cmos_sensor(0x0307, 0x8C);

	write_cmos_sensor(0x030D, 0x06);	// second_pre_pll_clk_div

	write_cmos_sensor(0x030E, 0x00);	// second_pll_multiplier
	write_cmos_sensor(0x030F, 0xAF);	// second_pll_multiplier
	write_cmos_sensor(0x0301, 0x04);	// vt_pix_clk_div
	...
	checksum_cal = 0;
	// check wb flag.
	// page 21 0x0a04 group 1, page 22 0x0a04 group 2, page 23 0x0a04 group 3
	for (i = 0x15; i <= 0x17; i++) {
		write_cmos_sensor(0x0A02, i);
		write_cmos_sensor(0x0A00, 0x01);
		mdelay(10);
		flag = read_cmos_sensor(0x0A04);
		if ((flag & 0xc0) >> 6 == 0x01) {
			addr = 0x0a05;
			break;
		}
	...
	LOG_INF("G-Dg wb page=%d, addr=0x%04X\n", i, addr);
	if (addr != 0) {
		for (i = 0; i < 30; i++) {
			s5k4h7_2a_data[i] = read_cmos_sensor(addr+i);
			checksum_cal += s5k4h7_2a_data[i];
			LOG_INF("G-Dg data=0x%02X, %d, checksum_cal=0x%04X\n", s5k4h7_2a_data[i], i, checksum_cal);
		}
		checksum_cal = checksum_cal % 255 + 1;
		if (checksum_cal != read_cmos_sensor(addr+30)) {
			pr_err("G-Dg s5k4h7 checksum fail, checksum_cal=0x%02X, read_checksum=0x%02X\n",
								checksum_cal, read_cmos_sensor(addr+30));
			for (i = 0; i < 30; i++) {
				s5k4h7_2a_data[i] = 0;
			}
			otp_status = 0;
	...
	checksum_cal = 0;
	// check af flag.
	// page 21 0x0a24 group 1, page 22 0x0a24 group 2, page 23 0x0a24 group 3
	for (i = 0x15; i <= 0x17; i++) {
		write_cmos_sensor(0x0A02, i);
		write_cmos_sensor(0x0A00, 0x01);
		mdelay(10);
		flag = read_cmos_sensor(0x0A24);
		if ((flag & 0xc0) >> 6 == 0x01) {
			addr = 0x0a25;
			break;
		}
	...
	LOG_INF("G-Dg af page=%d, addr=0x%04X\n", i, addr);
	if (addr != 0) {
		for (i = 0; i < 3; i++) {
			s5k4h7_2a_data[i+30] = read_cmos_sensor(addr+i);
			checksum_cal += s5k4h7_2a_data[i+30];
			LOG_INF("G-Dg data=0x%02X, %d, checksum_cal=0x%04X\n", s5k4h7_2a_data[i+30], i, checksum_cal);
		}
		checksum_cal = checksum_cal % 255 + 1;
		if (checksum_cal != read_cmos_sensor(addr+3)) {
			pr_err("G-Dg s5k4h7 checksum fail, checksum_cal=0x%02X, read_checksum=0x%02X\n",
								checksum_cal, read_cmos_sensor(addr+3));
			for (i = 0; i < 3; i++) {
				s5k4h7_2a_data[i+30] = 0;
			}
			otp_status = 0;
		} else {
			otp_status = 1;
