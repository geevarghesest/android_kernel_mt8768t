...
// P502
static char imgsensor_info[128] = {0};
unsigned int otp_status = 0;
		...
		if (pSensorList[drv_idx].init) {
			...
			if (psensor->pfunc) {
				...
				if (!imgsensor_check_is_alive(psensor)) {
					...
					// P502 imgsensor_info_show
					if ((psensor->inst.sensor_idx == 0) && (strstr(imgsensor_info, "REAR") || strstr(imgsensor_info, "FRONT")) ) {
						memset(imgsensor_info, 0, sizeof(imgsensor_info));
					}
					if (psensor->inst.sensor_idx == 0) {
						strcat(imgsensor_info, "REAR=");
						strcat(imgsensor_info, psensor_inst->psensor_name);
						strcat(imgsensor_info, ";");
						PK_DBG("G-Dg sensor_idx 0, imgsensor_info %s.\n", imgsensor_info);
					} else if (psensor->inst.sensor_idx == 1) {
						strcat(imgsensor_info, "FRONT=");
						strcat(imgsensor_info, psensor_inst->psensor_name);
						strcat(imgsensor_info, ";");
						PK_DBG("G-Dg sensor_idx 1, imgsensor_info %s.\n", imgsensor_info);
					}
					//
...
/************************************************************************
 * P502 create a node to show imgsensor info.
 ************************************************************************/
static struct kobject * sensor_kobject;
static ssize_t imgsensor_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	PK_DBG("G-Dg imgsensor_info_show imgsensor_info %s\n", imgsensor_info);
	sprintf(buf, "%s\n", imgsensor_info);
	PK_DBG("G-Dg imgsensor_info_show buf %s\n", buf);
	return strlen(buf);
}

static DEVICE_ATTR(sensorid, 0664, imgsensor_info_show, NULL);

// add for check sensor otp status
static ssize_t otp_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	sprintf(buf, "%s\n", otp_status ? "PASS" : "FAIL");
	PK_DBG("G-Dg otp_status_show buf=%s, status=%d\n", buf, otp_status);
	return strlen(buf);
}
static DEVICE_ATTR(otp_status, 0664, otp_status_show, NULL);
//
...
static int __init imgsensor_init(void)
{
	...
	if (sensor_kobject == NULL) {
		pr_err("[imgsensor_init]Big error: sensor_kobject_create_sysfs_ failed\n");
	} else {
		if (sysfs_create_file(sensor_kobject, &dev_attr_sensorid.attr) ||
			sysfs_create_file(sensor_kobject, &dev_attr_otp_status.attr)) {
