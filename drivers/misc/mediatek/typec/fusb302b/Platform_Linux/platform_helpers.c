...
FSC_S32 fusb_EnableInterrupts(void)
{
	...
	pr_err("FUSB  %s -  chip->gpio_IntN = '%d'\n", __func__, chip->gpio_IntN);
		...
		void *usr_ctx, void *app_ctx)
{
	...
	case CC_NO_ORIENT:
		...
		if(chip->partner != NULL) {
			typec_unregister_partner(chip->partner);
			chip->partner = NULL;
		}
		pr_info("FUSB %s:set to default role,current_pr=%d,current_dr=%d\n",
	...
	case PD_STATE_CHANGED:
		...
		if ((chip->port.PolicyState == peSinkReady ||  chip->port.PolicyState == peSourceReady ) &&
			chip->port.PolicyHasContract == TRUE) {
	...
	case DATA_ROLE:
		...
		if (chip->port.PolicyIsDFP == TRUE) {
			chip->current_dr = DR_DFP;
			typec_set_data_role(chip->typec_port, TYPEC_HOST);
			pr_info("FUSB - %s-%d: fusb302b PolicyIsDFP=0x%x, set typec_set_data_role", __func__, __LINE__, chip->port.PolicyIsDFP);
		} else {
			chip->current_dr = DR_UFP;
			typec_set_data_role(chip->typec_port, TYPEC_DEVICE);
			pr_info("FUSB - %s-%d: fusb302b PolicyIsDFP=0x%x, set typec_set_data_role", __func__,__LINE__, chip->port.PolicyIsDFP);
	...
	case POWER_ROLE:
		...
		if (chip->port.PolicyIsSource == FALSE) {
			chip->current_pr = PR_SINK;
			typec_set_pwr_role(chip->typec_port, TYPEC_SINK);
			pr_info("FUSB - %s-%d: fusb302b PolicyIsSource=0x%x, set typec_set_pwr_role", __func__, __LINE__, chip->port.PolicyIsSource);
