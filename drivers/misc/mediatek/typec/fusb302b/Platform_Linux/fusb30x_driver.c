...
static int fusbpd_typec_dr_set(const struct typec_capability *cap,
				enum typec_data_role role)
{
	struct fusb30x_chip* chip = container_of(cap, struct fusb30x_chip, typec_caps);
        pr_info("FUSB-fusbpd_typec_dr_set: setting data role to %d\n", role);
	pr_info("FUSB-%s PolicyIsDFP=%d\n", __func__, chip->port.PolicyIsDFP);
	...
	if (role == TYPEC_HOST) {
		if (chip->port.PolicyIsDFP == FALSE) {
			...
			pr_info("FUSB %s-%d: run pe---SendDRSwap\n", __func__, __LINE__);
		}
...
static int fusbpd_typec_pr_set(const struct typec_capability *cap,
				enum typec_role role)
{
	struct fusb30x_chip* chip = container_of(cap, struct fusb30x_chip, typec_caps);
        pr_info("fusbpd_typec_pr_set: setting power role to %d\n", role);

	pr_info("FUSB-%s PolicyIsSource=%d, PolicyState=%d\n", __func__,
	...
	if (role == TYPEC_SOURCE) {
		if (chip->port.PolicyState == peSinkReady) {
			if (chip->port.PolicyIsSource == FALSE) {
				...
				chip->port.PEIdle = FALSE;
				queue_work(chip->highpri_wq, &chip->sm_worker);
				pr_info("FUSB %s: run peSinkSendPRSwap\n", __func__);
			}
		} else if (chip->port.IsPRSwap ) {
			pr_info("FUSB %s: run PRSwap ...\n", __func__);
			return 0;
		} else {
			core_set_try_src(&chip->port);
		}
		pr_info("FUSB %s start try source or prswap to source \n", __func__);
		...
		if (chip->port.PolicyState == peSourceReady) {
			if (chip->port.PolicyIsSource == TRUE) {
				...
				chip->port.PEIdle = FALSE;
				queue_work(chip->highpri_wq, &chip->sm_worker);
				pr_info("FUSB %s: run peSourceSendPRSwap\n", __func__);
			}
		} else if (chip->port.IsPRSwap ) {
			pr_info("FUSB %s: run PRSwap ...\n", __func__);
			return 0;
		...
		pr_info("FUSB %s start try sink or prswap to sink \n", __func__);
...
static int fusbpd_typec_port_type_set(const struct typec_capability *cap,
				enum typec_port_type type)
{
	struct fusb30x_chip* chip = fusb30x_GetChip();

	pr_info("FUSB %s:power role state = %d\n", __func__, chip->port.sourceOrSink);
	pr_info("FUSB %s:new type=%d\n", __func__, type);
                          ...
                          const struct i2c_device_id* id)
{
    ...
    chip = devm_kzalloc(&client->dev, sizeof(struct fusb30x_chip), GFP_KERNEL);
    if (!chip)
	{
	...
	desc = devm_kzalloc(&client->dev, sizeof(struct tcpc_desc), GFP_KERNEL);
	if (!desc)
		return -ENOMEM;
    ...
    pr_info("FUSB  %s - GPIO initialized!\n", __func__);

    /* Initialize sysfs file accessors */
    fusb_Sysfs_Init();
    pr_info("FUSB  %s - Sysfs nodes created!\n", __func__);
    ...
    *  Interrupt must be enabled before starting 302 initialization */
	...
	schedule_delayed_work(&chip->reset_delay_work, msecs_to_jiffies(5000));
