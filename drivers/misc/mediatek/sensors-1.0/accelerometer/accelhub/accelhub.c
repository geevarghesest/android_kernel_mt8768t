...
static ssize_t cali_store(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(sscanf(buf, "%d %d %d %d %d %d", &data[0], &data[1], &data[2] ,&data[3], &data[4], &data[5])) {
		pr_err("[xpc]===cali_store: data: %d, %d, %d,%d, %d, %d \n", data[0], data[1], data[2], data[3], data[4], data[5]);
		res = sensor_set_cmd_to_hub(ID_ACCELEROMETER, CUST_ACTION_SET_CALI, &data[3]);
		if(res < 0) {
