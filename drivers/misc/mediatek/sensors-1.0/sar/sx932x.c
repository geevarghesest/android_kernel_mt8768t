...
int onoff_value = 0;//add for onoff 
int cal_flag = 0;//add for calibration
extern resume_flag;
static int gpiostatesar=2;
/*! \struct sx932x
			...
			struct device_attribute *attr,const char *buf, size_t count)
{
    ...
    if (!strncmp(buf, "0", 1))
    {
	 ...
	 write_register(this,0x11,0x20);
	 msleep(200);
	 onoff_value = 0 ;
	 pr_info("[SX932x]:onoff sar sensor is off \n");
    }
	if (!strncmp(buf, "1", 1))
    {
	//enable_irq(this->irq);
	 write_register(this,0x11,0x27);
	 msleep(200);
	 onoff_value = 1 ;
	 pr_info("[SX932x]:onoff on \n");
    }
...
{       
	...
	read_register(this,0XFA,&reg_value);
...
{  
	...
	read_register(this,0X01,&reg_value);
			...
			struct device_attribute *attr,const char *buf, size_t count)
{
    ...
    { 
	 write_register(this,0x11,0x20);
	 msleep(200);
	 pr_info("[SX932x]:calibration start  \n");
	 write_register(this,0x11,0x27);
	 msleep(200);
	 pr_info("calibration end \n");
	 cal_flag=1;
    }
	else
	{
...
int sxsar_calibration(psx93XX_t this)
{
#ifdef CONFIG_KERNEL_CUSTOM_FACTORY
	pr_info("sx9328 calibration in ear do nothing end \n");
	return 0;
#else
	 ...
	 write_register(this,0x11,0x27);
	 msleep(200);
	 input_report_key(input, pDevice->pbuttonInformation->buttons[1].keycode_release, 1);
	 input_sync(input);
	 input_report_key(input, pDevice->pbuttonInformation->buttons[1].keycode_release, 0);
	 input_sync(input);
	 pDevice->pbuttonInformation->buttons[1].state = IDLE;

	 input_report_key(input, pDevice->pbuttonInformation->buttons[2].keycode_release, 1);
	 input_sync(input);
	 input_report_key(input, pDevice->pbuttonInformation->buttons[2].keycode_release, 0);
	 input_sync(input);
	 pDevice->pbuttonInformation->buttons[2].state = IDLE;
	 pr_info("sx9328 calibration end \n");
	 return 0;
#endif
}
EXPORT_SYMBOL(sxsar_calibration);
...
#ifdef CONFIG_KERNEL_CUSTOM_FACTORY
		write_register(this,0x11,0x20);
		pr_info( "sx932x factory in init Done\n");
#else
		pr_info( "sx932x is notfactory in init Done\n");
#endif
		...
		for (counter = 0; counter < numberOfButtons; counter++) {
			...
			switch (pCurrentButton->state) {
				case IDLE: /* Button is not being touched! */
					if (((counter==1) || (counter==2)) && ((i & pCurrentButton->mask) == pCurrentButton->mask)) {
						/* User pressed button */
					 ...
					 else if ((counter==0) && ((i & pCurrentButton->mask) == pCurrentButton->mask))
					{
						dev_info(this->pdev, "cap button %d touched  but donot report\n", counter);
						pCurrentButton->state = ACTIVE;
					}
					else {
				...
				case ACTIVE: /* Button is being touched! */ 
					...
					{
						/* User released button */
						dev_info(this->pdev,"Button %d released but donot report\n",counter);
						pCurrentButton->state = IDLE;
					}
					else {
...
static int sx932x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("sx read_register for Addr: 0x%02x Return: 0x%02x\n",address, returnValue);

	if (value == returnValue) {
						pr_info("SX9328 ID detect success!\n");
				}
	else{
						pr_info("SX9328 ID detect error!\n");
						sar_flag=3;
						return -EINVAL;
		}
...
#ifdef CONFIG_KERNEL_CUSTOM_FACTORY
	write_register(this,0x11,0x20);
	pr_info( "sx932x_probe() factory Done\n");
#else
	pr_info( "sx932x_probe() Done\n");
#endif
...
static void sx93XX_schedule_work(psx93XX_t this, unsigned long delay)
{
	...
	if (this) {
		...
		dev_info(this->pdev, "sx93XX_schedule_work() end \n");
	}
	else
...
static void sx93XX_worker_func(struct work_struct *work)
{
	...
	int counter = 0;
	u8 nirqLow = 0;
	//dev_info(this->pdev, "sx93XX_work_func() start \n");
	if (work) {
...
void sx93XX_suspend(psx93XX_t this)
{
	if (this){
			...
			write_register(this,0x11,0x20);
			resume_flag=2;
			gpiostatesar=gpio_get_value(irq_gpio_num);
			pr_info("[SX932x]:onoff sar sensor is off gpio irq is %d\n",gpiostatesar);
			pr_info("sx9328 suspend resume flage is 2 \n");
	}
...
#ifdef CONFIG_KERNEL_CUSTOM_FACTORY
		write_register(this,0x11,0x20);
		pr_info("sx932x resume factory in resume Done sar is off \n"); 
#else
		pr_info("sx932x is not factory in resume Done\n");
		if(resume_flag==1)
		{
			write_register(this,0x11,0x20);
			//msleep(200);
			gpiostatesar=gpio_get_value(irq_gpio_num);
			pr_info("sx9328 resume but the resume is wake up by rtc so do not turn on SAR gpio state is %d  \n",gpiostatesar);
		}
		else{
				//write_register(this,0x11,0x20);
				//sx93XX_IRQ_init(this);
				...
				pr_info("sx9328 resume end SAR Is on gpio irq is %d \n",gpiostatesar);
			}
