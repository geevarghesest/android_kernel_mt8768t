...
//	static struct device_node *of_node;
	...
	if (!inited) {
		...
		return;
	#endif
	/*2020.08.27 longcheer xugui usb_phy_tuning start*/
	if (mtk_musb->is_host) {
	...
	/*2020.08.27 longcheer xugui usb_phy_tuning end*/
	if (u2_vrt_ref != -1) {
...
void usb_phy_recover(struct musb *musb)
{
	...
	usb_phy_tuning();
	/* 7 (111b => VRT_VREF_SEL = 820)
	   6 (110b => TERM_VREF_SEL = 440) */
//	USBPHY_SET8(0x05, 0x76);
