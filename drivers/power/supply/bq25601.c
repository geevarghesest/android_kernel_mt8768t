...
static bool g_bq25601_vbus_en = 0;
static int bq25601_otg_discharge_pin = 0;
...
static int bq25601_set_hizmode(struct charger_device *chg_dev, bool en)
{
	int status = 0;
...
static int bq25601_enable_discharge(struct charger_device *chg_dev, bool en)
{
	int ret = 0;

	pr_info("%s en = %d\n", __func__, en);
	if (en) {
		gpio_direction_output(bq25601_otg_discharge_pin,1);
		mdelay(20);
	} else {
		gpio_direction_output(bq25601_otg_discharge_pin,0);
	}
	return ret;
}
...
static unsigned int charging_hw_init(void)
{
	...
	bq25601_set_en_hiz(0x0);
	bq25601_set_vindpm(0x2);	/* VIN DPM check 4.6V-4.1v */
	bq25601_set_wdt_rst(0x1);	/* Kick watchdog */
	...
	bq25601_set_ovp(0x3);	/* set ovp to 14V */
	bq25601_set_treg(0x3);	/* set otg to 5.3V */
	bq25601_set_chg_config(0x0);/*disable charging for PD certification*/
	bq25601_set_batfet_delay(0x0);
			    ...
			    struct device *dev)
{
	...
	bq25601_otg_discharge_pin = of_get_named_gpio(np,"ti,bq2560x,otg-discharge-gpio",0);
	if(bq25601_otg_discharge_pin < 0){
	 	pr_info("%s: no bq25601_otg_discharge_pin\n", __func__);
		return -ENODATA;
	}
	else{
		pr_info("%s:__func__, bq25601_otg_discharge_pin %d\n", __func__,bq25601_otg_discharge_pin);
		gpio_request(bq25601_otg_discharge_pin,"bq25601_otg_discharge_en");
	}
...
static struct charger_ops bq25601_chg_ops = {
	...
	.enable_discharge = bq25601_enable_discharge,
