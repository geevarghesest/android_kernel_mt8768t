				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "temp_t2_thres_minus_x_degree", &val) >= 0)
		info->data.temp_t2_thres_minus_x_degree = val;
	else {
		chr_err("use default TEMP_T2_THRES_MINUS_X_DEGREE:%d\n",
			TEMP_T2_THRES_MINUS_X_DEGREE);
		info->data.temp_t2_thres_minus_x_degree =
					TEMP_T2_THRES_MINUS_X_DEGREE;
	}
	...
	g_usb_charge_disable_gpio = of_get_named_gpio(np, "usb_charge_disable_gpio", 0);
	if (g_usb_charge_disable_gpio < 0){
		chr_err("%s can not get g_usb_charge_disable_gpio\n",__func__);
	}
	else{
		gpio_request(g_usb_charge_disable_gpio, "usb_charge_disable");
	}
}
...
void do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		if ((sw_jeita->sm == TEMP_ABOVE_T4)
		    ...
		    && (info->battery_temp
			<= info->data.temp_t3_thres_plus_x_degree)){
			chr_err("[SW_JEITA] Battery Temperature between %d and %d,%d,%d,%d!!\n",
				info->data.temp_t3_thres,
				info->data.temp_t3_thres_plus_x_degree,
				sw_jeita->pre_sm,sw_jeita->sm,sw_jeita->charging);
	...
	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d en:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		sw_jeita->cv, sw_jeita->charging);
}
...
static ssize_t BatteryPresent_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int bat_present;

	bat_present = gauge_get_int_property(GAUGE_PROP_BATTERY_EXIST);

	chr_err("%s: %d\n", __func__, bat_present);
	return sprintf(buf, "%d\n", bat_present);
}

static DEVICE_ATTR_RO(BatteryPresent);

bool charging_enabled = true;
int charging_enable_flag = 0;
void mtk_charging_enable_write(int en)
{
...
EXPORT_SYMBOL(mtk_charging_enable_write);
EXPORT_SYMBOL(charging_enabled);
EXPORT_SYMBOL(charging_enable_flag);
...
void usb_charge_gpio_enable(bool en){
	bool status = false;
	//status = gpio_get_value(g_usb_charge_disable_gpio);

	chr_err("%s,status=%d,en=%d\n",__func__,status,en);
	#if 0
	if(en){
		if (0 == status)
		gpio_direction_output(g_usb_charge_disable_gpio, 1);
	}
	else{
		if (1 == status)
		gpio_direction_output(g_usb_charge_disable_gpio, 0);
	}
	#endif
}
EXPORT_SYMBOL(usb_charge_gpio_enable);

int g_soc_state = BAT_PROTECT_SOC_UNKNOWN;
static void charger_check_status(struct mtk_charger *info)
{
	bool charging = true;
	bool typec_charging = true;
	bool soc_charging = true;
	int temperature;
	...
	if(true == g_batt_protect_mode){
	uisoc = get_uisoc(info);
	if (uisoc > 60){
		//mtk_charging_enable_write(0);
		//update_battery_status(psy,0);
		soc_charging = false;
		charger_dev_set_hizmode(info->chg1_dev, true);//enter hiz mode,battery offer power
		g_soc_state = BAT_PROTECT_SOC_ABOVE_60;
	}
	else if (uisoc < 40){
		//mtk_charging_enable_write(1);
		//update_battery_status(psy,1);
		//soc_charging = true;
		g_soc_state = BAT_PROTECT_SOC_BELOW_40;
	}
	else{
		if (g_soc_state == BAT_PROTECT_SOC_ABOVE_60){
		charger_dev_set_hizmode(info->chg1_dev, false);//exit hiz mode,charger offer power
		soc_charging = false;
		}else if (g_soc_state == BAT_PROTECT_SOC_BELOW_40){
			if(uisoc == 60){
			soc_charging = false;
			charger_dev_set_hizmode(info->chg1_dev, false);//exit hiz mode,charger offer power
	...
	if (info->vbusov_stat)
		charging = false;
	if (typec_charging == false)
		charging = false;
	if (soc_charging == false)
		charging = false;
	if ((voltage >= 4200000) && (temperature >= 45)){
		//charging = false;
		g_high_temp_charging = false;
		}
	if (g_high_temp_charging == false){
		charging = false;
		if(((voltage < 4000000) && (temperature >= 45))||(temperature < 45)){
			g_high_temp_charging = true;
			charging = true;
		}
	}
...
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d can:%d typec:%d vol:%d Ten:%d soc:%d\n",
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout,
		info->vbusov_stat, info->can_charging, typec_temperature, voltage, g_high_temp_charging, uisoc);

	if (charging != info->can_charging)
		_mtk_enable_charging(info, charging);

	if(false == charging)
		update_battery_charge_status(psy,0);
	else
		update_battery_charge_status(psy,1);
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	/* Battery Present */
	ret = device_create_file(&(pdev->dev), &dev_attr_BatteryPresent);
	if (ret)
		goto _out;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (chg == info->chg1_dev)
			val->intval =
				get_battery_temperature(info);//info->chg_data[CHG1_SETTING].junction_temp_max;
