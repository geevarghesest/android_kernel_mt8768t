...
//Connect a 10k resistor in series in DVT. 1:0ohm 2:100kohm
#define MAIN_UP		24000
#define MAIN_DOWM	100
#define SLAVE_UP	120000
#define SLAVE_DOWM	24000
...
static int get_battery_type(void)
{
	...
	if((batt_id > MAIN_DOWM) && (batt_id < MAIN_UP))
		return POWER_SUPPLY_BATTERY_TYPE_MAIN;
	else if ((batt_id > SLAVE_DOWM) && (batt_id < SLAVE_UP))
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		if(g_charge_cycle_count > 0)
		val->intval = g_charge_cycle_count;
		else
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		/* 6= ATE_factory_boot */
		if (gm->bootmode == 4 //|| gm->bootmode == 4
...
void update_battery_charge_status(struct power_supply *psy, int val)
{
	struct mtk_battery *gm;
	struct battery_data *bs_data;
	...
	const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		g_charge_cycle_count = val->intval;
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
...
void battery_service_data_init(struct mtk_battery *gm)
{
	...
	bs_data->psd.property_is_writeable = battery_property_is_writeable;
