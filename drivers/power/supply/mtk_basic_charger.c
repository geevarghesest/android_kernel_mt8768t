...
static int charge_cycle_count = -1;
static int g_sw_jeita_cv = -1;
static int g_data_battery_cv = -1;
...
static void select_cv(struct mtk_charger *info)
{
	...
	if (psy)
	rc = power_supply_get_property(psy,POWER_SUPPLY_PROP_CYCLE_COUNT,&pval);

	//if(charge_cycle_count != pval.intval){
		charge_cycle_count = pval.intval;
	if (info->enable_sw_jeita)
		if (info->sw_jeita.cv != 0) {
			if(g_sw_jeita_cv < 0)
				g_sw_jeita_cv = info->sw_jeita.cv;
		if(charge_cycle_count > 400){
			info->setting.cv = g_sw_jeita_cv - 200000;
		}
		else if (charge_cycle_count > 150 && charge_cycle_count <= 400){
			info->setting.cv = g_sw_jeita_cv - 50000;
		}else{
			info->setting.cv = g_sw_jeita_cv;
		}
		//info->setting.cv = info->sw_jeita.cv;
			chr_err("%s,cv:%d,cycle:%d\n",__func__,info->setting.cv,charge_cycle_count);
			return;
		}

	if(g_data_battery_cv < 0)
		g_data_battery_cv = info->data.battery_cv;

	if(charge_cycle_count > 400){
		constant_voltage = g_data_battery_cv - 200000;
	}
	else if (charge_cycle_count > 150 && charge_cycle_count <= 400){
		constant_voltage = g_data_battery_cv - 50000;
	}
	else{
		constant_voltage = g_data_battery_cv;
	}
	//constant_voltage = info->data.battery_cv;
	info->setting.cv = constant_voltage;
	//}

	chr_err("%s,cv:%d,cyclee:%d\n",__func__,info->setting.cv,charge_cycle_count);
}
	...
	struct chg_limit_setting *setting)
{
	...
	} else if ((info->chr_type == POWER_SUPPLY_TYPE_USB_DCP)||(info->chr_type == POWER_SUPPLY_TYPE_POGO)) {
		pdata->input_current_limit =
			info->data.ac_charger_input_current;
		pdata->charging_current_limit =
			info->data.ac_charger_current;
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			if (info->sw_jeita.sm == TEMP_T0_TO_T1) {
				...
				pdata->input_current_limit = 1100000;
				pdata->charging_current_limit = 1000000;
				chr_err("select_charging_current_limit: 45~50 degree\n");
			}
		}
	}
