...
//Add for battery id start
#define BAT_ID_VOLT 1800000000
#define BAT_ID_UP_R 100000
static int batt_id_get(struct iio_channel *chan)
{
	...
	id_r = id_v / id_i;
	printk("batt_id_get chg:v=%d,i=%d,r=%d\n",id_v,id_i,id_r);
	return id_r;
}
//Add for battery id end
...
static int mt6357_gauge_probe(struct platform_device *pdev)
{
	...
	struct iio_channel *chan_batt_id;  //Add for bat_id
